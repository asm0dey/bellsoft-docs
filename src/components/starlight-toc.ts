...
// Distinct name from Starlight's own `starlight-toc` (still used by the mobile
// ToC) to avoid a duplicate custom-element registration error.
if (!customElements.get('starlight-toc-collapsible')) {
  customElements.define('starlight-toc-collapsible', StarlightTOC);
}
