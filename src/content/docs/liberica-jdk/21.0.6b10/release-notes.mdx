...
description: Release notes for Liberica JDK 21.0.6+10 (LTS) — what's new, CRaC support, fixed CVEs, and resolved issues.
---

import { Aside } from '@astrojs/starlight/components';

_Published: January 21, 2025_

## Version information

This document provides information about the Liberica JDK 21.0.6 release. The full version string for this update release is 21.0.6+10. The version number is 21.
...
| --- | --- |
| JDK-8337966 | **Files.readAttributes fails with Operation not permitted on older docker releases.** JDK used to fail with `java.nio.file.FileSystemException: Operation not permitted` on old docker versions (before v19). The regression was introduced by JDK-8316304 adding a `statx` syscall. On old docker versions, `statx` is not added to the default seccomp white list, causing `EPERM` in some systems even if the container image was updated. |
| JDK-8339581 | **`{ClassLoading,Memory}MXBean::isVerbose` methods are inconsistent with their setVerbose methods.** The `ClassLoadingMXBean` and `MemoryMXBean` APIs have `setVerbose` methods to control verbose mode and `isVerbose` methods to query it. Some JCK tests expect `setVerbose(false)` to disable verbose mode and `isVerbose()` to subsequently return false. However, if logging to a file was enabled via `-Xlog` on the launcher command line, `isVerbose()` returned true even after calling `setVerbose(false)`. |
| JDK-8339907 | **Distrust TLS server certificates issued after Oct 2024 and anchored by Entrust Root CAs.** Google and Mozilla have announced plans to distrust TLS server certificates issued by Entrust. This enhancement implements similar restrictions in the JDK, enforced in the SunJSSE Provider of the JSSE API. A TLS session will not be negotiated if the server's certificate chain is anchored by any of the specified CAs and the certificate's `notBefore` date is after October 31, 2024. You can work around the restrictions by removing `ENTRUST_TLS` from the `jdk.security.caDistrustPolicies` security property. |
| JDK-8339908 | **BasicFileAttributes.creationTime() falls back to epoch if birth time is unavailable (Linux).** The change resolving JDK-8316304 added support for obtaining the file creation time from the birth timestamp. Previously, the last modified time was returned as the creation time. After this change, if the birth timestamp was unavailable (for example, unsupported by the file system even though the OS supported it), the epoch (0) was returned, which could cause problems in situations relying on the old behavior. |
| JDK-8341054 | **Hotspot should be able to use more than 64 logical processors on Windows.** Hotspot can now use processors across all Windows processor groups on Windows 11 / Windows Server 2022 and later. The `GetSystemInfo` API was replaced by `GetActiveProcessorCount` to correctly retrieve the total number of processors, and a new `UseAllWindowsProcessorGroups` Windows-specific product flag was added to enable this functionality. |
| JDK-8341392 | **Add 2 SSL.com TLS roots.** New SSL.com TLS roots are added as they are the issuing CA used by Entrust going forward. |
| JDK-8341393 | **Change Entrust TLS distrust date to November 12, 2024.** Google changed their distrust date for Entrust from November 1 to November 12, 2024. Entrust also changed the date they plan to use SSL.com as the issuing CA to November 12. To align, the distrust date was changed to November 12, 2024. |
| JDK-8343828 | **Generic type information lost on mandated parameters of record's compact constructors.** Reflection was not retrieving generic type information for mandated parameters. For compact record constructors, the generic type information is now fetched from the Signature attribute for its parameters, independently of whether they are mandated. This is a core-reflection only change; the compilation of records with compact constructors is not affected. |
| JDK-8344678 | **Update Timezone Data to 2024b.** This release upgrades the in-tree copy of the IANA timezone database to 2024b, primarily improving historical data for Mexico, Mongolia, and Portugal. It also makes Asia/Choibalsan an alias for Asia/Ulaanbaatar and makes the MET timezone the same as CET. |
| JDK-8345437 | **Add an operation mode to the jar command when extracting to not overwrite existing files.** Previously, when the jar tool extracted files, it would overwrite any existing files with the same name. A new option (`-k` or `--keep-old-files`) can now be specified so that existing files are not overwritten — for example, `jar xkf foo.jar` or `jar --extract --keep-old-files --file foo.jar`. By default, the old behavior remains and files are overwritten. |

### Builds of Liberica JDK With CRaC

This release of Liberica JDK includes full CRaC support. CRaC support is available for Liberica JDK 17 and 21. For more information, see [Using CRaC with Java applications](https://docs.bell-sw.com/liberica-jdk/latest/how-to/using-crac/).

### IANA TZ Data update

This release of Liberica JDK 21.0.6 upgrades the in-tree copy of the [IANA timezone database](https://www.iana.org/time-zones) to 2024b. This timezone update is primarily concerned with improving historical data for Mexico, Mongolia, and Portugal. It also makes Asia/Choibalsan an alias for Asia/Ulaanbaatar and makes the MET timezone the same as CET.

The 2024b update also makes a number of legacy timezone IDs equal to geographical names rather than fixed offsets:

- `EST` → `America/Panama` instead of -5:00
- `MST` → `America/Phoenix` instead of -7:00
- `HST` → `Pacific/Honolulu` instead of -10:00

For long-term support releases of Liberica JDK, this change is overridden locally to retain the existing fixed offset mapping.
...
| JDK-8335714 | Enhance playing MP3s |
| JDK-8335715 | Improve Direct Show support |
| JDK-8336798 | DRT test cssrounding.html test for linear layout fails with WebKit 619.1 |
| JDK-8336941 | Update libxslt to 1.1.42 |
...
| JDK-8340208 | Additional WebKit 619.1 fixes from WebKitGTK 2.44.4 |
| JDK-8341920 | Intermittent WebKit build failure on Windows generating PDB files in 619.1 |
| JDK-8343136 | Change JavaFX release version to 21.0.6 in jfx21u |
...
To keep your Liberica JDK up-to-date and secure, always upgrade to the newest available version once it is released. To upgrade, install the new version over the previous one. For installation instructions, see the [Liberica JDK Installation Guide](/bellsoft-docs/liberica-jdk/21.0.6b10/install-guide/).
